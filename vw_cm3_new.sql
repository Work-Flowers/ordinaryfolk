...
cm1 AS (
	SELECT
	  ...
	  SUM(amount * gst_vat) AS gst_vat,
	  SUM(amount * fee_rate) AS payment_gateway_fees,
	  SUM(amount * refund_rate) AS refunds
	FROM base
	GROUP BY 1,2,3,4,5
),
...
SELECT
	...
	SAFE_DIVIDE(cm1.revenue, 
	  SUM(cm1.revenue) OVER (PARTITION BY cm1.acq_month, cm1.country)
	) * m.cost AS prorated_marketing_cost
FROM cm1
